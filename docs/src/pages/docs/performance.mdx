...
We use standard OpenTelemetry libraries in our implementation to ensure that our tool does not introduce any performance overhead. We accept there to me minor overhead from our implementation as we add additional instrumentation to the server. We work hard to ensure GraphQL Debuggers performance does not impact the performance of your server.

## Benchmarks

We have public benchmarks executed on each commit to ensure that we are not introducing any performance regressions. You can find the benchmarks in our monorepo under the `benchmarks` directory.
