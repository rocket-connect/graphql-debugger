import { forwardRef } from "react";

import type { IconProps, IconRef } from "./types";

export const Docs = forwardRef((props: IconProps, ref: IconRef) => (
  <svg
    ref={ref}
    viewBox="0 0 512 512"
    {...props}
    height={props.height || 30}
    width={props.width || 30}
    className={props.className || "fill-neutral-100"}
  >
    <path d="M447.1 96H275l-48.3-45.25C214.7 38.74 198.5 32 181.5 32H63.1c-35.35 0-64 28.66-64 64v320c0 35.34 28.65 64 64 64h384c35.35 0 64-28.66 64-64V160c0-35.3-27.8-64-64-64zm16 320c0 8.824-7.178 16-16 16h-384c-8.822 0-16-7.176-16-16V96c0-8.824 7.178-16 16-16h117.5c4.273 0 8.293 1.664 11.31 4.688L255.1 144h192c8.822 0 16 7.176 16 16v256z" />
  </svg>
));

export const DocsFilled = forwardRef((props: IconProps, ref: IconRef) => (
  <svg
    ref={ref}
    viewBox="0 0 512 512"
    {...props}
    height={props.height || 30}
    width={props.width || 30}
    className={props.className || "fill-neutral-100"}
  >
    <path d="M448 480H64c-35.3 0-64-28.7-64-64V192h512v224c0 35.3-28.7 64-64 64zm64-320H0V96c0-35.3 28.7-64 64-64h117.5c17 0 33.3 6.7 45.3 18.7l26.5 26.5c12 12 28.3 18.7 45.3 18.7H448c35.3 0 64 28.7 64 64z" />
  </svg>
));
