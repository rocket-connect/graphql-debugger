import { Query, Schema, ListSchemasWhere } from '../graphql-types';
import { api } from './api';
...
export async function listSchemas({ where }: { where?: ListSchemasWhere } = {}): Promise<Schema[]> {
  const { data, errors } = await api<{ listSchemas: Query['listSchemas'] }>({
